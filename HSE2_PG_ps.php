  ...
  if(isset($_POST["submit"])){
    ...
    if(isset($findings_PG) || $findings_PG!=""){
      for($i=0;$i<$quanity3;$i++){
        ...
        if($status_PG[$i]==2 && $time_frame_PG[$i]==NULL){
          $time_frame_PG[$i]=1;
        }

        $sql = "IF NOT EXISTS (SELECT * FROM MarSIS_DW_Findings
        WHERE FINDINGS = '$findings_PG[$i]' AND INSPECTION_ID ='$INSPECTION_ID')
        ...
        echo "<script type= 'text/javascript'>location.href = 'index.php?page=HSE2_PG' </script>";
