  ...
  if(isset($_POST["submit"])){
    ...
    if(isset($findings_fuel) || $findings_fuel!=""){
      for($i=0;$i<$quanity3;$i++){
        ...
        if($status_fuel[$i]==2 && $time_frame_fuel[$i]==NULL){
          $time_frame_fuel[$i]=1;
        }

        $no=$i;
        $sql = "IF NOT EXISTS (SELECT * FROM MarSIS_DW_Findings
        WHERE FINDINGS = '$findings_fuel[$i]' AND INSPECTION_ID ='$INSPECTION_ID')
        ...
        echo "<script type= 'text/javascript'>location.href = 'index.php?page=HSE1_fuel' </script>";
