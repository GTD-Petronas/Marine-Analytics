...
if($type==1){
  ...
  if(isset($_POST["submit"])){
    ...
    $findings_other = $_POST['findings_other'];
    $status_other = $_POST['status_other'];
    $time_frame_other = $_POST['time_frame_other'];
    $result_other = array_filter($findings_other);
    $quanity = count($result_other);


    if(isset($findings_other) || $findings_other!=""){
      for($i=0;$i<$quanity;$i++){
        $no=$i;
        ...
        if($status_other[$i]==2 && $time_frame_other[$i]==NULL){
          $time_frame_other[$i]=1;
        }

        $sql = "IF NOT EXISTS (SELECT * FROM MarSIS_DW_Findings
        WHERE FINDINGS = '$findings_other[$i]' AND INSPECTION_ID ='$INSPECTION_ID')
        ...
        VALUES ('$no',16,NULL,'$status_other[$i]','$time_frame_other[$i]','$FINDING_ID_OVID','$findings_other[$i]',NULL,'$INSPECTION_ID',NULL,NULL,'$user','$update_time')";
            $stmt = sqlsrv_query($conn, $sql);


          }
        }
        echo "<script type= 'text/javascript'>location.href = 'index.php?page=Other_other' </script>";
