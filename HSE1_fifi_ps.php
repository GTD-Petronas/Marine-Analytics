  ...
  if(isset($_POST["submit"])){
    ...
    if(isset($findings_fifi) || $findings_fifi!=""){
      for($i=0;$i<$quanity6;$i++){
        ...
        if($status_fifi[$i]==2 && $time_frame_fifi[$i]==NULL){
          $time_frame_fifi[$i]=1;
        }

        $sql = "IF NOT EXISTS (SELECT * FROM MarSIS_DW_Findings
        WHERE FINDINGS = '$findings_fifi[$i]' AND INSPECTION_ID ='$INSPECTION_ID')
    ...
    echo "<script type= 'text/javascript'>location.href = 'index.php?page=HSE1_fifi' </script>";
