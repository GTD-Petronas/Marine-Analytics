...
WHERE INSPECTION_ID='$INSPECTION_ID' AND a.INS_ITEM_ID IN ('6')";
...
<section id="main">
  <div class="container">
    <div class="row">
      <div class="col-md-10">
        ...
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            ...
            <form action="HSE1_vts_edit_ps.php" method="POST" enctype="multipart/form-data">
                  ...
                  <table id="dataTable_ovid" class="table table-bordered table-responsive">
                    ...
                    if(!empty($count)){
                      ...
                      while( $row = sqlsrv_fetch_array( $stmtInspectionI, SQLSRV_FETCH_ASSOC) ) {
                        ...
                        echo'
                        <tr>
                        <td><textarea name="findings_vts[]"  id="finding_vts'.$row['FINDINGS_ID'].'" cols="65">'.$row['FINDINGS'].'</textarea></td>
                        <td><select id="status_vts'.$row['FINDINGS_ID'].'" name="status_vts[]">
                        <option value=""></option>';
                        ...
                        <td><select id="time_frame_vts'.$row['FINDINGS_ID'].'" name="time_frame_vts[]">
                        <option value=""></option>';
                      ...
                      ?>
                      <tr>
                        <td><textarea name="findings_vts[]"  cols="65"></textarea></td>
                        <td><select id="status_vts[]" name="status_vts[]">
                          <option value=""></option>
                          <?php
                          $sql="SELECT * FROM MarSIS_MDM_Status WHERE STATUS_ID IN ('2','3')";
                          $stmt = sqlsrv_query( $conn, $sql);
                      ...
                      <td><select id="time_frame" name="time_frame_vts[]">
                        <option value=""></option>
                        <?php
                        $sql1="SELECT * FROM MarSIS_MDM_TimeFrame";
                        $stmt1 = sqlsrv_query( $conn, $sql1);
      ...
      <button onclick="location.href='index.php?page=HSE1_fifi_edit'" class="btn btn-md pull-right" style="background-color:#40E0D0; margin-right:5px; ">Next</button>
...
function delete_data(finding_id){
  var finding = document.getElementById("finding_vts"+finding_id).value;
  var time_frame = document.getElementById("time_frame_vts"+finding_id).value;
  var status = document.getElementById("status_vts"+finding_id).value;
  var type = 1; //delete
  ...
  xmlhttp.open("GET","HSE1_vts_edit_ps.php?finding_id="+ finding_id + "&type="+type + "&finding="+finding + "&time_frame="+time_frame + "&status="+status,true);
  xmlhttp.send();
}
</script>
