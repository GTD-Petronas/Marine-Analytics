  ...
  if(isset($_POST["submit"])){
    ...
    if(isset($findings_FFA) || $findings_FFA!=""){
      for($i=0;$i<$quanity1;$i++){
        ...
        if($status_FFA[$i]==2 && $time_frame_FFA[$i]==NULL){
          $time_frame_FFA[$i]=1;
        }

        $sql = "IF NOT EXISTS (SELECT * FROM MarSIS_DW_Findings
        WHERE FINDINGS = '$findings_FFA[$i]' AND INSPECTION_ID ='$INSPECTION_ID')
        ...
        echo "<script type= 'text/javascript'>location.href = 'index.php?page=HSE2_FFA_edit' </script>";
