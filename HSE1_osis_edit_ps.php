...
if($type==1){
  ...
  if(isset($_POST["submit"])){
    ...
    $findings_osis = $_POST['findings_osis'];
    $pax_no_osis = $_POST['pax_no_osis'];
    $status_osis = $_POST['status_osis'];
    $time_frame_osis = $_POST['time_frame_osis'];
    $result_osis = array_filter($findings_osis);
    $quanity2 = count($result_osis);

    if(isset($findings_osis) || $findings_osis!=""){
      for($i=0;$i<$quanity2;$i++){
        ...
        if($status_osis[$i]==2 && $time_frame_osis[$i]==NULL){
          $time_frame_osis[$i]=1;
        }

        $no=$i;
        $sql = "IF NOT EXISTS (SELECT * FROM MarSIS_DW_Findings
        WHERE FINDINGS = '$findings_osis[$i]' AND INSPECTION_ID ='$INSPECTION_ID')
        ...
        VALUES ('$no',3,'$pax_no_osis[$i]','$status_osis[$i]','$time_frame_osis[$i]','$FINDING_ID_OVID','$findings_osis[$i]',NULL,'$INSPECTION_ID',NULL,NULL,'$user','$update_time')";
            $stmt = sqlsrv_query($conn, $sql);


          }
        }
        echo "<script type= 'text/javascript'>location.href = 'index.php?page=HSE1_osis_edit' </script>";
