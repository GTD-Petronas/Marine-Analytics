  ...
  <div class="container">
    <div class="row">
                          ...
                          <table id="dataTable_cap" class="table table-bordered table-responsive">
                            ...
                            <tr>
                              ...
                              while( $row = sqlsrv_fetch_array( $stmtInspectionI, SQLSRV_FETCH_ASSOC) ) {
                                ...
                                if($duedate!=NULL){
                                  echo'<td><input type="date" name="trans_date" id="cap_duedate'.$row['FINDINGS_ID'].'"  value="'.date_format($row['DUE_DATE'], 'Y-m-d').'" disabled></td>';
                                }elseif($type_of_inspection==2 && $duedate==NULL){
                                  echo'<td><input type="date" name="trans_date" id="cap_duedate'.$row['FINDINGS_ID'].' " value="'.$hiredate.'" disabled></td>';
                                }else{
      ...
      function save_data_all(finding_id){
        ...
        //   }
        // };
        // xmlhttp.open("GET","corrective_action_plan_ps.php?finding_id="+ finding_id + "&duedate="+ duedate+ "&date_closure="+ date_closure+ "&textarea="+textarea +"&status="+ status,true);
        // xmlhttp.send();
