  ...
  if(isset($_POST["submit"])){
    ...
    if(isset($findings_Quart) || $findings_Quart!=""){
      for($i=0;$i<$quanity5;$i++){
        ...
        if($status_Quart[$i]==2 && $time_frame_Quart[$i]==NULL){
          $time_frame_Quart[$i]=1;
        }

        $sql = "IF NOT EXISTS (SELECT * FROM MarSIS_DW_Findings
        WHERE FINDINGS = '$findings_Quart[$i]' AND INSPECTION_ID ='$INSPECTION_ID')
        ...
        echo "<script type= 'text/javascript'>location.href = 'index.php?page=HSE2_Quart_edit' </script>";
