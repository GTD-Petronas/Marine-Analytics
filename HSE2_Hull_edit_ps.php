  ...
  if(isset($_POST["submit"])){
    ...
    if(isset($findings_Hull) || $findings_Hull!=""){
      for($i=0;$i<$quanity6;$i++){
        ...
        if($status_Hull[$i]==2 && $time_frame_Hull[$i]==NULL){
          $time_frame_Hull[$i]=1;
        }

        $sql = "IF NOT EXISTS (SELECT * FROM MarSIS_DW_Findings
        WHERE FINDINGS = '$findings_Hull[$i]' AND INSPECTION_ID ='$INSPECTION_ID')
        ...
        echo "<script type= 'text/javascript'>location.href = 'index.php?page=HSE2_Hull_edit' </script>";
