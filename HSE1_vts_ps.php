  ...
  if(isset($_POST["submit"])){
    ...
    if(isset($findings_vts) || $findings_vts!=""){
      for($i=0;$i<$quanity5;$i++){
        ...
        if($status_vts[$i]==2 && $time_frame_vts[$i]==NULL){
          $time_frame_vts[$i]=1;
        }

        $sql = "IF NOT EXISTS (SELECT * FROM MarSIS_DW_Findings
        WHERE FINDINGS = '$findings_vts[$i]' AND INSPECTION_ID ='$INSPECTION_ID')
        ...
        echo "<script type= 'text/javascript'>location.href = 'index.php?page=HSE1_vts' </script>";
